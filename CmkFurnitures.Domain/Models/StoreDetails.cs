        ...
        public List<string> GetPrettyOpeningHours()
        {
            ...
            foreach (var groupedHour in groupedHours)
            {
                ...
                if (orderedHours.Count(x => x.Closeing == hour.Closeing && x.Opening == hour.Opening) > 1)
                {
                    ...
                    returnList.Add(string.Format("{0}-{3} fra {1:00:00}-{2:00:00}", dayNames[startDay.Day], hour.Opening, hour.Closeing, dayNames[endDay.Day]));
                }
                else
                {
                    returnList.Add(string.Format("{0} fra {1:00:00}-{2:00:00}", dayNames[hour.Day], hour.Opening, hour.Closeing));
                }
            }
